...
class TdeedVideoClip:
    ...
    def from_video_clip(
        ...
        labels2int_map = cls.get_labels2int_map(labels_enum, team_aware=team_aware)
        nr_of_classes = (len(labels_enum) * 2 + 1) if team_aware else (len(labels_enum) + 1)
        ...
        for idx, frame in enumerate(video_clip.frames):
            if frame.annotation:
                ...
                if team_aware:
                    ...
                    if frame.annotation.label not in labels2int_map:
                        continue
    ...
    @staticmethod
    def get_labels2int_map(labels_enum: Type[enum.Enum], team_aware: bool = True):
        if team_aware:
        ...
        return {label: i + 1 for i, label in enumerate(labels_enum)}

    @staticmethod
    def get_int2label_map(labels_enum: Type[enum.Enum], team_aware: bool = True):
        m = TdeedVideoClip.get_labels2int_map(labels_enum, team_aware=team_aware)
...
class TeamTDeedDataset(Dataset):
    ...
    def random_train_val_split(
        ...
        random_seed: int = 42,
    ) -> tuple["TeamTDeedDataset", "TeamTDeedDataset"]:
        """Random clip split for one (or few) videos where split_by_matches counts do not apply."""
        ...
        if n == 1:
            ds = TeamTDeedDataset(clips, **kw)
            return ds, ds
        n_val = max(1, min(n - 1, int(round(n * val_fraction))))
        val_clips = clips[:n_val]
        train_clips = clips[n_val:]
        return (
            TeamTDeedDataset(train_clips, **kw),
            TeamTDeedDataset(val_clips, **kw),
        )
...
class TeamTDeed2HeadsPrediction:
    ...
    @staticmethod
    def compute_action_scores_matrix(
        video: SoccerVideo,
        predictions: List["TeamTDeed2HeadsPrediction"],
        labels_enum: Type[enum.Enum],
        no_background: bool = False,
    ) -> np.ndarray:
        """Average clip logits over time for team-agnostic (subnet 44) heads."""

        scores_matrix = np.zeros(
            shape=(video.actual_n_frames, len(labels_enum) + 1), dtype=np.float32
        )
        ...
        for prediction in tqdm(predictions, desc="constructing action scores matrix"):
            preds = prediction.aligned_team_label_predictions
            ...
            scores_matrix[start_frame : (end_frame + 1)] = (
                scores_matrix[start_frame : (end_frame + 1)] + preds
            )
