...
from dudek.ml.model.tdeed.eval.subnet44_eval import Subnet44TDeedEvaluator
...
def train(
    experiment_name: str,
    model: TDeedModule,
    labels_enum: Type[BASLabel | ActionLabel | ScorevisionAction],
    ...
    if eval_metric == "map" and val_dataset is not None:
        if team_aware:
            ...
            evaluator = Subnet44TDeedEvaluator(
                model=model,
                dataset=val_dataset,
                delta_frames_tolerance=subnet44_delta_frames,
            )
    ...
    with torch.no_grad() if evaluate else nullcontext():
        for batch in tqdm(data_loader, total=len(data_loader)):
            ...
            with torch.amp.autocast(device):
                ...
                n_classes = (2 * len(labels_enum) + 1) if team_aware else (len(labels_enum) + 1)
                predictions = pred.reshape(-1, n_classes)
