...
LabelEnum = Union[BASLabel, ActionLabel, ScorevisionAction]
...
class SoccerVideo:
    ...
    labels_class: Type[LabelEnum] = dataclasses.field(default=None)
    ...
    @staticmethod
    def _parse_bas_mp4_resolution(filename: str) -> Optional[int]:
        m = re.fullmatch(r"(\d+)p\.mp4", filename, flags=re.IGNORECASE)
        return int(m.group(1)) if m else None

    @classmethod
    def bas_video_from_flat_folder(cls, folder_path: str, resolution: int) -> "SoccerVideo":
        """
        Single-match layout: one folder containing ``{resolution}p.mp4`` and label JSON
        (``Labels-scorevision.json`` or ``Labels-ball.json``), no league/season/match tree.
        """

        folder_path = os.path.normpath(os.path.abspath(folder_path))
        match_label = Path(folder_path).name
        video_name = f"{resolution}p.mp4"
        absolute_path = os.path.join(folder_path, video_name)
        if not os.path.isfile(absolute_path):
            found = sorted(
                f
                for f in os.listdir(folder_path)
                if cls._parse_bas_mp4_resolution(f) is not None
            )
            raise FileNotFoundError(
                f"Missing {video_name!r} under {folder_path!r}. "
                f"Use --resolution to match your file (found: {found or 'no *p.mp4'})."
            )

        scorevision_path = os.path.join(folder_path, "Labels-scorevision.json")
        ball_path = os.path.join(folder_path, "Labels-ball.json")
        annotations = None
        labels_class: Type[LabelEnum] = BASLabel
        if os.path.exists(scorevision_path):
            annotations = Annotation.load_annotations(
                scorevision_path, enum_class=ScorevisionAction
            )
            labels_class = ScorevisionAction
        elif os.path.exists(ball_path):
            annotations = Annotation.load_bas_annotations(ball_path)
    ...
    def bas_video_from_path(cls, video_dir_path: str, resolution: int) -> "SoccerVideo":
        ...
        parts = Path(video_dir_path).resolve().parts
        if len(parts) >= 3:
            season_name, league_name, match_label = parts[-3], parts[-2], parts[-1]
        else:
            season_name, league_name, match_label = "_flat", "_flat", parts[-1] if parts else "video"
        ...
        scorevision_path = os.path.join(video_dir_path, "Labels-scorevision.json")
        ball_path = os.path.join(video_dir_path, "Labels-ball.json")
        annotations = None
        labels_class: Type[LabelEnum] = BASLabel
        if os.path.exists(scorevision_path):
            annotations = Annotation.load_annotations(
    ...
    def with_scorevision_subnet44_annotations_from_bas(self) -> "SoccerVideo":
        """Remap BAS labels to Score Vision (subnet 44) actions; drops unmapped events."""

        from dudek.data.bas_scorevision_mapping import bas_label_to_scorevision_action

        if self.labels_class is not BASLabel:
            raise ValueError(
                "with_scorevision_subnet44_annotations_from_bas requires labels_class=BASLabel"
            )
        new_ann: List[Annotation] = []
        for a in self.annotations or []:
            if not isinstance(a.label, BASLabel):
                continue
            sv = bas_label_to_scorevision_action(a.label)
            if sv is None:
                continue
            new_ann.append(
                Annotation(
        ...
        return SoccerVideo(
            ...
            annotations=new_ann or None,
            half=self.half,
            labels_class=ScorevisionAction,
            cached_initial_frame=self.cached_initial_frame,
        )
    ...
    def compute_action_labels_matrix(
        self,
        labels_enum: Type[enum.Enum],
        no_background: bool = False,
    ) -> np.ndarray:
        """Per-frame multi-hot over action classes (no left/right split)."""

        labels2int_map = {label: i + 1 for i, label in enumerate(labels_enum)}
        labels_matrix = np.zeros(
            (self.actual_n_frames, len(labels_enum) + 1), dtype=np.float32
        )
        if self.annotations:
            for annotation in self.annotations:
                if annotation.label not in labels2int_map:
                    continue
                col = labels2int_map[annotation.label]
                frame_nr = annotation.get_frame_nr(fps=self.metadata_fps)
                if 0 <= frame_nr < labels_matrix.shape[0]:
                    labels_matrix[frame_nr, col] = 1.0
        return labels_matrix if not no_background else labels_matrix[:, 1:]
