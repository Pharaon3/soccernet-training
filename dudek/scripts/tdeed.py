...
from dudek.utils.ml import load_state_dict_matching_shapes
from dudek.utils.video import (
    load_action_spotting_videos,
    load_bas_videos,
    load_bas_videos_scorevision_subnet44,
)
...
@cli.command("train-subnet44")
@click.option("--dataset_path", type=str, required=True)
@click.option("--resolution", type=int, default=224)
@click.option("--clip_frames_count", type=int, default=80)
@click.option("--overlap", type=int, default=60)
@click.option("--displacement", type=int, default=4)
...
@click.option("--even_choice_proba", type=float, default=0.0)
@click.option("--nr_epochs", type=int, default=25)
@click.option("--warm_up_epochs", type=int, default=1)
@click.option("--learning_rate", type=float, default=0.0006)
@click.option("--train_batch_size", type=int, default=8)
@click.option("--val_batch_size", type=int, default=8)
...
@click.option(
    ...
    type=click.Choice(["none", "backbone_only", "matching_shapes"]),
    default="matching_shapes",
    help="How to use --model_checkpoint_path when set (e.g. SN-BAS 12x2-head ckpt).",
)
@click.option(
    "--subnet44_delta_frames",
    type=int,
    default=25,
    help="Validation mAP tolerance in frames (~1s at 25 FPS; miner uses per-class seconds).",
)
@click.option("--save_as", type=str, default="tdeed_subnet44.pt")
@click.option("--experiment_name", type=str, default="tdeed_subnet44")
@click.option("--random_seed", type=int, default=TEST_SET_CHALLENGE_SEED)
@click.option("--use_wandb", type=bool, default=False)
def train_subnet44(
    dataset_path: str,
    ...
    tdeed_arch_sgp_k: int = 4,
    model_checkpoint_path: str = None,
    checkpoint_load_mode: str = "matching_shapes",
    subnet44_delta_frames: int = 25,
    save_as: str = "tdeed_subnet44.pt",
    experiment_name: str = "tdeed_subnet44",
    random_seed: int = TEST_SET_CHALLENGE_SEED,
    use_wandb: bool = False,
):
    """
    Fine-tune T-DEED for Bittensor subnet 44 (Score Vision private track).

    Uses SN-BAS videos with BAS labels mapped to Score Vision actions (see
    dudek.data.bas_scorevision_mapping). Miner output format: frame index + action
    string per https://github.com/score-technologies/turbovision/blob/main/scorevision/miner/private_track/MINER.md
    """
    assert resolution in [224, 720]
    assert eval_metric in ("loss", "map")
    if use_wandb:
        wandb.init(project=experiment_name, sync_tensorboard=True)

    videos = load_bas_videos_scorevision_subnet44(dataset_path, resolution=resolution)
    all_clips = []
    for v in tqdm(videos, desc="Loading clips ..."):
    ...
    if n_matches == 7:
        train_dataset, val_dataset, _test_dataset = all_dataset.split_by_matches(
            counts=[4, 1, 2], random_seed=random_seed
        )
    else:
        train_dataset, val_dataset = all_dataset.random_train_val_split(
            val_fraction=0.15, random_seed=random_seed
        )
    ...
    num_action_classes = len(ScorevisionAction)
    tdeed_model = TDeedModule(
        clip_len=clip_frames_count,
        n_layers=tdeed_arch_n_layers,
        sgp_ks=tdeed_arch_sgp_ks,
    ...
    if model_checkpoint_path and checkpoint_load_mode != "none":
        if checkpoint_load_mode == "backbone_only":
            tdeed_model.load_backbone(model_checkpoint_path)
        elif checkpoint_load_mode == "matching_shapes":
            load_state_dict_matching_shapes(tdeed_model, model_checkpoint_path)
    ...
    train_tdeed(
        ...
        warm_up_epochs=warm_up_epochs,
        team_aware=False,
        subnet44_delta_frames=subnet44_delta_frames,
    )
